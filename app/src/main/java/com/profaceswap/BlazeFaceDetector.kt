...
class BlazeFaceDetector(private val session: OrtSession) {
    ...
    fun detect(bitmap: Bitmap): List<BlazeFaceResult> {
        ...
        val scale =
            INPUT_SIZE.toFloat() / max(originalWidth, originalHeight).toFloat()
        ...
        Canvas(inputBitmap).drawBitmap(bitmap, matrix, null)

        val inputBuffer = createBlazeFaceInput(inputBitmap)
        ...
        val regressors = result[0].value as Array<*>
        val scores = result[1].value as Array<*>

        val reg = regressors[0] as Array<*>
        val scoreArray = scores[0] as Array<*>
        ...
        for (i in 0 until ANCHOR_COUNT) {
            ...
            val values = reg[i] as FloatArray

            val anchorX = ((i % 16) + 0.5f) / 16f
            val anchorY = (((i / 16) % 16) + 0.5f) / 16f

            val cx = values[0] / INPUT_SIZE + anchorX
            val cy = values[1] / INPUT_SIZE + anchorY

            val width = values[2] / INPUT_SIZE
            val height = values[3] / INPUT_SIZE
        ...
        val selected = weightedNms(candidates)
    ...
    private fun createBlazeFaceInput(bitmap: Bitmap): FloatBuffer {
    ...
    private fun unletterbox(
        ...
        return ((value * INPUT_SIZE - padding) / scale)
            .coerceIn(0f, maximum)
    }
    ...
    private fun weightedNms(
        ...
        while (remaining.isNotEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (iou(top, candidate) > IOU_THRESHOLD) {
            ...
            for (detection in overlapping) {
                ...
                val weight = detection.score

                totalWeight += weight

                x1 += detection.x1 * weight
                y1 += detection.y1 * weight
                x2 += detection.x2 * weight
                y2 += detection.y2 * weight

                score = max(score, detection.score)
            }
